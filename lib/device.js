...
module.exports = function (Cam) {
   ...
   Cam.prototype.setNTP = function (options, callback) {
      ...
      let body = this._envelopeHeader() + '<SetNTP xmlns="http://www.onvif.org/ver10/device/wsdl">' + '<FromDHCP>' + options.fromDHCP + '</FromDHCP>';
   ...
   Cam.prototype.setNetworkInterfaces = function (options, callback) {
      ...
      let body =
         ...
         ni.enabled +
         '</Enabled>' +
         (ni.link ? '<Link xmlns="http://www.onvif.org/ver10/schema">' + '<AutoNegotiation>' + ni.link.autoNegotiation + '</AutoNegotiation>' + '<Speed>' + ni.link.speed + '</Speed>' + '<Duplex>' + ni.link.duplex + '</Duplex>' + '</Link>' : '') +
         (!isNaN(ni.MTU) ? '<MTU xmlns="http://www.onvif.org/ver10/schema">' + ni.MTU + '</MTU>' : '') +
         (ni.IPv4 ? '<IPv4 xmlns="http://www.onvif.org/ver10/schema">' + '<Enabled>' + ni.IPv4.enabled + '</Enabled>' + '<Manual>' + '<Address>' + ni.IPv4.manual.address + '</Address>' + '<PrefixLength>' + ni.IPv4.manual.prefixLength + '</PrefixLength>' + '</Manual>' + '<DHCP>' + ni.IPv4.DHCP + '</DHCP>' + '</IPv4>' : '') +
         (ni.IPv6 ? '<IPv6 xmlns="http://www.onvif.org/ver10/schema">' + '<Enabled>' + ni.IPv6.enabled + '</Enabled>' + '<AcceptRouterAdvert >' + ni.IPv6.acceptRouterAdvert + '</AcceptRouterAdvert>' + '<Manual>' + '<Address>' + ni.IPv6.manual.address + '</Address>' + '<PrefixLength>' + ni.IPv6.manual.prefixLength + '</PrefixLength>' + '</Manual>' + '<DHCP>' + ni.IPv6.DHCP + '</DHCP>' + '</IPv6>' : '') +
         '</NetworkInterface>' +
   ...
   Cam.prototype.getNetworkDefaultGateway = function (callback) {
      this._request(
         {
            service: 'device',
            body: this._envelopeHeader() + '<GetNetworkDefaultGateway xmlns="http://www.onvif.org/ver10/device/wsdl"/>' + this._envelopeFooter(),
         },
         function (err, data, xml) {
            if (callback) {
               if (!err) {
   ...
   Cam.prototype.setNetworkDefaultGateway = function (options, callback) {
      this._request(
         {
            service: 'device',
            body: this._envelopeHeader() + '<SetNetworkDefaultGateway xmlns="http://www.onvif.org/ver10/device/wsdl">' + (options.IPv4Address ? '<IPv4Address>' + options.IPv4Address + '</IPv4Address>' : '') + (options.IPv6Address ? '<IPv6Address>' + options.IPv6Address + '</IPv6Address>' : '') + '</SetNetworkDefaultGateway>' + this._envelopeFooter(),
         },
         function (err, data, xml) {
            if (callback) {
               if (err) {
   ...
   Cam.prototype.setDNS = function (options, callback) {
      let body = this._envelopeHeader() + '<SetDNS xmlns="http://www.onvif.org/ver10/device/wsdl">' + '<FromDHCP>' + !!options.fromDHCP + '</FromDHCP>' + (options.searchDomain ? '<SearchDomain>' + options.searchDomain + '</SearchDomain>' : '');
      options.DNSManual.forEach(function (dns) {
         body += '<DNSManual>' + '<Type xmlns="http://www.onvif.org/ver10/schema">' + (dns.type === 'IPv6' ? 'IPv6' : 'IPv4') + '</Type>' + (dns.IPv4Address ? '<IPv4Address xmlns="http://www.onvif.org/ver10/schema">' + dns.IPv4Address + '</IPv4Address>' : '') + (dns.IPv6Address ? '<IPv6Address xmlns="http://www.onvif.org/ver10/schema">' + dns.IPv6Address + '</IPv6Address>' : '') + '</DNSManual>';
      });
   ...
   /**
    * @callback Cam~GetDynamicDNSCallback
    * @property {?Error} error
    * @property {Array.<Cam~dynamicDNSInformation>} DynamicDNS information
    * @property {string} xml Raw SOAP response
    */

   /**
    * Get dynamicDNS information
    * @param {Cam~GetDynamicDNSCallback} [callback]
    */
   Cam.prototype.getDynamicDNS = function (callback) {
      this._request(
         {
            service: 'device',
            body: this._envelopeHeader() + '<GetDynamicDNS xmlns="http://www.onvif.org/ver10/device/wsdl"/>' + this._envelopeFooter(),
         },
         function (err, data, xml) {
            if (callback) {
               if (!err) {
                  this.dynamicDNSInformation = linerase(data[0].getDynamicDNSResponse[0]);
               }
               callback.call(this, err, this.dynamicDNSInformation, xml);
            }
         }.bind(this)
      );
   };
   ...
   /**
    * Set Dynamic DNS information
    * @param {Cam~DynamicDNSInformation} options
    * @param {Cam~getDynamicDNSCallback} [callback]
    */
   Cam.prototype.setDynamicDNS = function (options, callback) {
      let body = this._envelopeHeader() + '<SetDynamicDNS xmlns="http://www.onvif.org/ver10/device/wsdl">' + '<Type>' + !!options.type + '</Type>' + (options.Name ? '<Name>' + options.name + '</Name>' : '') + (options.ttl ? '<TTL>' + options.ttl + '</TTL>' : '');
      body += '</SetDynamicDNS>' + this._envelopeFooter();
      this._request(
         {
            service: 'device',
            body: body,
   ...
   Cam.prototype.createUsers = function (users, callback) {
      ...
      users.forEach(function (user) {
         usersXml += '<User>' + '<Username xmlns="http://www.onvif.org/ver10/schema">' + user.username + '</Username>' + '<Password xmlns="http://www.onvif.org/ver10/schema">' + user.password + '</Password>' + '<UserLevel xmlns="http://www.onvif.org/ver10/schema">' + user.userLevel + '</UserLevel>' + '</User>';
      });
      var body = this._envelopeHeader() + '<CreateUsers xmlns="http://www.onvif.org/ver10/device/wsdl">' + usersXml + '</CreateUsers>' + this._envelopeFooter();
      this._request(
         {
            service: 'device',
            body: body,
   ...
   Cam.prototype.deleteUsers = function (users, callback) {
      ...
      var body = this._envelopeHeader() + '<DeleteUsers xmlns="http://www.onvif.org/ver10/device/wsdl">' + usersXml + '</DeleteUsers>' + this._envelopeFooter();
      this._request(
         {
            service: 'device',
            body: body,
